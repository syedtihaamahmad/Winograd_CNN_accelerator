...
_ffi.cdef("""
void load_layer(unsigned int addr);
void deinit();
"""
)
...
#         print(array)
    ...
    def set_io(self, file):
        ...
        self.output_buffer = allocate(shape=out_shape, dtype=np.uint32)
        
        np.copyto(self.input_buffer, image)
        np.copyto(self.output_buffer, np.zeros(out_shape, dtype=np.uint32))
