...
const unsigned int totalLayers = 5;

const layer C0 = {0,
		OFM_CH_C0/out_parallel_C0, IMG_CH/simd_C0, pe_num_C0,
		has_bias_C0, OFM_CH_C0/out_parallel_C0, out_parallel_C0};

const layer C1 = {1,
		OFM_CH_C1/out_parallel_C1, OFM_CH_C0/simd_C1, pe_num_C1,
...
const layer C3 = {3,
		OFM_CH_C3/out_parallel_C3, OFM_CH_C2/simd_C3, pe_num_C3,
		has_bias_C3, OFM_CH_C3/out_parallel_C3, out_parallel_C3};

const layer C4 = {4,
		OFM_CH_C4/out_parallel_C4, OFM_CH_C3/simd_C4, pe_num_C4,
		has_bias_C4, OFM_CH_C4/out_parallel_C4, out_parallel_C4};

layer layers[totalLayers] = {C0, C1, C2, C3, C4};

unsigned long long W_C0[OFM_CH_C0/out_parallel_C0][IMG_CH/simd_C0][pe_num_C0] = {
#include "./data/conv0_packed_out1.txt"
};

unsigned long long W_C1[OFM_CH_C1/out_parallel_C1][OFM_CH_C0/simd_C1][pe_num_C1]={
#include "./data/conv1_packed_out1.txt"
};

unsigned long long W_C2[OFM_CH_C2/out_parallel_C2][OFM_CH_C1/simd_C2][pe_num_C2]={
#include "./data/conv2_packed_out1.txt"
};

unsigned long long W_C3[OFM_CH_C3/out_parallel_C3][OFM_CH_C2/simd_C3][pe_num_C3] = {
#include "./data/conv3_packed_out1.txt"
};

unsigned long long W_C4[OFM_CH_C4/out_parallel_C4][OFM_CH_C3/simd_C4][pe_num_C4] = {
#include "./data/conv4_packed_out1.txt"
};


//Biases are signed because they are not packed

long long B_C0[OFM_CH_C0/out_parallel_C0][out_parallel_C0] = {
#include "./data/qconv0_b.txt"
};
...
long long B_C3[OFM_CH_C3/out_parallel_C3][out_parallel_C3] = {
#include "./data/qconv3_b.txt"
};

long long B_C4[OFM_CH_C4/out_parallel_C4][out_parallel_C4] = {
#include "./data/qconv4_b.txt"
};
...
void Load_Weights(XNn_top *xbbj)
{
		...
		for(unsigned int i=0; i<l.weight_ch; i++)
			for(unsigned int r=0; r<l.weight_row; r++)
				for(unsigned int c=0; c<l.weight_col; c++)
					Load_Val_Weight(xbbj, 3, i,r,c, W_C1[i][r][c]);
		...
		for(unsigned int i=0; i<l.weight_ch; i++)
			for(unsigned int r=0; r<l.weight_row; r++)
				for(unsigned int c=0; c<l.weight_col; c++)
					Load_Val_Weight(xbbj, 5, i,r,c, W_C2[i][r][c]);

		l = layers[3];
		xil_printf("Initializing weights layer 3\n");
		for(unsigned int i=0; i<l.weight_ch; i++)
			for(unsigned int r=0; r<l.weight_row; r++)
				for(unsigned int c=0; c<l.weight_col; c++)
					Load_Val_Weight(xbbj, 7, i,r,c, W_C3[i][r][c]);

		l = layers[4];
		xil_printf("Initializing weights layer 4\n");
		for(unsigned int i=0; i<l.weight_ch; i++)
			for(unsigned int r=0; r<l.weight_row; r++)
				for(unsigned int c=0; c<l.weight_col; c++)
					Load_Val_Weight(xbbj, 9, i,r,c, W_C4[i][r][c]);
...
void Load_Bias(XNn_top *xbbj)
{
		...
		layer l = layers[0];
		xil_printf("Initializing bias layer 0\n");
				for(unsigned int r=0; r<l.bias_row; r++)
					for(unsigned int c=0; c<l.bias_col; c++)
						Load_Val_Bias(xbbj, 2, 0,r,c, B_C0[r][c]);

		l = layers[1];
		xil_printf("Initializing bias layer 1\n");
				for(unsigned int r=0; r<l.bias_row; r++)
					for(unsigned int c=0; c<l.bias_col; c++)
						Load_Val_Bias(xbbj, 4, 0,r,c, B_C1[r][c]);

		l = layers[2];
		xil_printf("Initializing bias layer 2\n");
				for(unsigned int r=0; r<l.bias_row; r++)
					for(unsigned int c=0; c<l.bias_col; c++)
						Load_Val_Bias(xbbj, 6, 0,r,c, B_C2[r][c]);

		l = layers[3];
		xil_printf("Initializing bias layer 3\n");
				for(unsigned int r=0; r<l.bias_row; r++)
					for(unsigned int c=0; c<l.bias_col; c++)
						Load_Val_Bias(xbbj, 8, 0,r,c, B_C3[r][c]);

		l = layers[4];
		xil_printf("Initializing bias layer 4\n");
				for(unsigned int r=0; r<l.bias_row; r++)
					for(unsigned int c=0; c<l.bias_col; c++)
						Load_Val_Bias(xbbj, 10, 0,r,c, B_C4[r][c]);
