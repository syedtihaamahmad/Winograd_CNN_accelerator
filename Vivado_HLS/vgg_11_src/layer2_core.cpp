...
//};
	...
	weights_C1 = new ap_uint<simd_C0*bitw> **[l.ch];
		for(int r=0; r< l.ch;r++){
			weights_C1[r] = new ap_uint<simd_C0*bitw> *[l.row];

			for(int col=0; col<l.row; col++)
				weights_C1[r][col] = new ap_uint<simd_C0*bitw> [l.col];
		}
	...
	StreamingDataWidthConverter_Batch<TO::width, TO_32::width, (OFM_DIM_C1*OFM_DIM_C1*OFM_CH_C1)/out_parallel_C1>(c1_outstream, outstream, reps);
	Stream2Mem<TO_32::width,(OFM_DIM_C1*OFM_DIM_C1*OFM_CH_C1)/(out_parallel_C1*4) >(outstream, out, reps);
...
//		break;
//	case 4:
//		bias_C1[target_row][target_col] = val(B_C2::width-1,0);
//		break;
	}
}
